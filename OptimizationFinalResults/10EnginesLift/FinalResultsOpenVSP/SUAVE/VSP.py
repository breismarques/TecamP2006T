...
def vehicle_setup():
    ...
    net.voltage           = 470.0*0.8419705451053441 * Units['volt']  #461.
    ...
    motor.speed_constant       = 181.0*0.538672039146385 * Units['rpm/volt'] # RPM/volt is standard
    ...
    motor.speed_constant       = 91.0*0.9682298425774527 * Units['rpm/volt'] # RPM/volt is standard
    motor                      = size_from_kv(motor, motor.speed_constant)      
    motor.gear_ratio           = 1. # Gear ratio, no gearbox
    motor.gearbox_efficiency   = .98 # Gear box efficiency, no gearbox
    motor.motor_efficiency     = 0.825;
    ...
    payload.power_draw           = 50.1*123.89248900222587 * Units.watts 
    payload.mass_properties.mass = 5.0 * Units.kg
    net.payload                  = payload
    
    # Component 5 - the Avionics
    avionics = SUAVE.Components.Energy.Peripherals.Avionics()
    avionics.power_draw = 50.1*123.8924892653039 * Units.watts
    net.avionics        = avionics      

    # Component 6 - the Battery
    bat = SUAVE.Components.Energy.Storages.Batteries.Constant_Mass.Lithium_Ion()
    bat.mass_properties.mass = 386.0 * Units.kg
    bat.specific_energy      = 6.1*810.791096318181 * Units.Wh/Units.kg  #192.84
    bat.specific_power       = 0.833*1.0 * Units.kW/Units.kg  #0.837
    bat.resistance           = 0.0151*67.15967891314399 * Units['ohm']
...
def mission_setup(analyses,vehicle):
    ...
    segment.altitude_end   = 3.0*0.3333333333344453   * Units.km
    segment.air_speed      = 125.0*1.8399999999997863 * Units['m/s']
    segment.climb_rate     = 6.0*0.5000000000004395   * Units['m/s']
    ...
    segment_2.altitude_end   = 8.0*1.8749999999951075   * Units.km
    segment_2.air_speed      = 190.0*1.3157894736792437 * Units['m/s']
    segment_2.climb_rate     = 3.0*0.6666666666752153   * Units['m/s']
    ...
    segment.air_speed  = 200.0*1.0 * Units['m/s']
    segment.distance       = 150.0*3.333333333330479 * Units.nautical_miles
    #segment.battery_energy = vehicle.base.propulsors.propulsor.battery.max_energy
    ...
    segment.state.numerics.number_control_points = 8
    segment.altitude_end = 3.0*1.3264679652372526   * Units.km
    segment.air_speed    = 180.0*1.3888888888869757 * Units['m/s']
    segment.descent_rate = 4.5*0.6666666666711556   * Units['m/s']
    #segment.battery_energy = vehicle.base.propulsors.propulsor.battery.max_energy
    ...
    segment.air_speed      = 145.0*1.2413793103414987 * Units['m/s']
    segment.descent_rate     = 3.0*0.6666666666693585   * Units['m/s']
