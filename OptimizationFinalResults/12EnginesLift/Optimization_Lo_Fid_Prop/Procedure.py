...
def post_process(nexus):
    ...
    energy.extend(conditions.climb_1.propulsion.battery_energy[:,0]/Units.Wh)
    energy.extend(conditions.climb_2.propulsion.battery_energy[:,0]/Units.Wh)
    energy.extend(conditions.cruise.propulsion.battery_energy[:,0]/Units.Wh)
    energy.extend(conditions.descent_1.propulsion.battery_energy[:,0]/Units.Wh)
    energy.extend(conditions.descent_2.propulsion.battery_energy[:,0]/Units.Wh)
    
    summary.max_battery_energy_all_segments = np.max(energy)
    summary.min_battery_energy_all_segments = np.min(energy)
    
    file.write('Battery Energy = '+str(energy)+'\n')
