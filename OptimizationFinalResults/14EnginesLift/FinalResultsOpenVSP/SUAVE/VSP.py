...
def vehicle_setup():
    ...
    net.voltage           = 470.0*0.2370522275978431 * Units['volt']  #461.
    ...
    motor.speed_constant       = 181.0 *1.5073391780285084 * Units['rpm/volt'] # RPM/volt is standard
    ...
    motor.speed_constant       = 91.0*1.1808599926002727 * Units['rpm/volt'] # RPM/volt is standard
    motor                      = size_from_kv(motor, motor.speed_constant)    
    motor.gear_ratio           = 1. # Gear ratio, no gearbox
    motor.gearbox_efficiency   = .98 # Gear box efficiency, no gearbox
    motor.motor_efficiency     = 0.825;
    ...
    payload.power_draw           = 50.1 * 0.7420275479139963 * Units.watts 
    payload.mass_properties.mass = 5.0 * Units.kg
    net.payload                  = payload
    
    # Component 5 - the Avionics
    avionics = SUAVE.Components.Energy.Peripherals.Avionics()
    avionics.power_draw = 50.1 * 0.7420275479067225 * Units.watts
    net.avionics        = avionics      

    # Component 6 - the Battery
    bat = SUAVE.Components.Energy.Storages.Batteries.Constant_Mass.Lithium_Ion()
    bat.mass_properties.mass = 386.0 * Units.kg
    bat.specific_energy      = 6.1*1.9516805346981712 * Units.Wh/Units.kg  #192.84
    bat.specific_power       = 0.833*1.0 * Units.kW/Units.kg  #0.837
    bat.resistance           = 0.0151 * 0.9297246923253196 * Units['ohm']
...
def mission_setup(analyses,vehicle):
    ...
    segment.altitude_end   = 3.0*0.33333333333333326   * Units.km
    segment.air_speed      = 125.0*1.84 * Units['m/s']
    segment.climb_rate     = 6.0*0.5   * Units['m/s']
    ...
    segment_2.climb_rate     = 3.0*3.0   * Units['m/s']
    ...
    segment.air_speed  = 200.0*0.45 * Units['m/s']
    segment.distance       = 150.0*0.3333333333333335 * Units.nautical_miles
    #segment.battery_energy = vehicle.base.propulsors.propulsor.battery.max_energy
    ...
    segment.state.numerics.number_control_points = 8
    segment.altitude_end = 3.0*0.6666666666666665   * Units.km
    segment.air_speed    = 180.0*0.5555555555555556 * Units['m/s']
    segment.descent_rate = 4.5*2.2222222222222223   * Units['m/s']
    #segment.battery_energy = vehicle.base.propulsors.propulsor.battery.max_energy
    ...
    segment.air_speed      = 145.0*0.3448275862068966 * Units['m/s']
    segment.descent_rate     = 3.0*2.6666666666666665  * Units['m/s']
